...
const Repositories = ({ className, ...props }) => {
    ...
    const setCursors = (queryResult) => {
        ...
        const {
            startCursor,
            endCursor,
        } = queryResult.user.repositories.pageInfo;
        setEndCursor(endCursor);
        setStartCursor(startCursor);
    };

    const [getUserRepositories, { loading, error, data }] = useLazyQuery(
        SEARCH_REPO_QUERY,
        {
    ...
    const userHasNoRepository =
        !data.user || !data.user.repositories.nodes.length;
