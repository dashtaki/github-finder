...
const UsersList = ({ className, ...props }) => {
    ...
    if (userNotFound)
        return (
            <div className={className}>
                <h1 className="user-not-found__text">No Users found!</h1>
            </div>
        );
    ...
    return (
        <ul className={className}>
            {users &&
                users.search.edges.map((user) => (
                    <li
                        key={user.node.id}
                        onClick={() => handleOnUserClick(user.node)}
                    >
                        <img
                            width="40"
                        ...
                        <div className="user-detail__wrapper">
                            <small className="user-detail__name">
                                {user.node.name || 'No Name'}
                            </small>
                            <small>
                                Followers: {user.node.followers.totalCount}{' '}
                            </small>
                            <small>
                                Following: {user.node.following.totalCount}
                            </small>
                        </div>
                    </li>
                ))}
